...
//	LCD_TK80_init();  //初始化液晶屏相关GPIO
	...
	mp_hal_delay_ms(100);
	SPI_WriteComm(0x3A); //pixel format setting
	SPI_WriteData(0x77); 
	SPI_WriteComm(0x36); //pixel format setting
	SPI_WriteData(0x60); //B,G,R
}
...
void lcd43g_init(void)
{
	...
	lcddev.width  = X_PIXEL; //480;
	lcddev.height = Y_PIXEL; //800;
	lcddev.backcolor = BLACK;
	
	for(uint32_t i=0; i < X_PIXEL * Y_PIXEL; i++){
		LTDC_Buf[i] = 0x0U;
	}
	LCD_Fill_Pic(0, 0, X_PIXEL, Y_PIXEL, (u32*)LTDC_Buf);
...
void LCD_Color_Fill(uint16_t sx, uint16_t sy, uint16_t ex, uint16_t ey, uint16_t *color)
{
  ...
  for(y = sy; y < ey+sy; y++)
  {
	...
	for (y = sy; y < ey+sy; y++) {
		for (x=sx; x < ex+sx; x++) {
			LTDC_Buf[x + y * lcddev.width] =
						((color_dat & 0xF800)<<8) | ((color_dat & 0x7E0)<<5) | ((color_dat & 0x1F)<<3);
		//LCD_Fast_DrawPoint(x,y,((color_dat & 0xF800)<<8) | ((color_dat & 0x7E0)<<5) | ((color_dat & 0x1F)<<3));
		}
	}
	LCD_Fill_Pic(sx, sy, ex - sx, ey - sy, (u32*)LTDC_Buf);
#endif
} 
...
/*
	...
	LCD_Fill_Pic(sx, sy, ex - sx, ey - sy, (u32*)LTDC_Buf);
*/
	Lcd_ColorBox(sx, sy, ex - sx, ey - sy, color);
#endif
}
//=============================================================================
	...
	for (t = 0; t < csize; t++) {
		...
		for (t1 = 0; t1 < 8; t1++) {
			if (temp & 0x80)
				...
				LCD_Fast_DrawPoint(x, y, lcddev.backcolor);	//back color
...
{
//	Lcd_ColorBox(0, 0, lcddev.height, lcddev.width, color);
	Lcd_ColorBox(0, 0, X_PIXEL, Y_PIXEL, color);
}
...
void LCD_Display_Dir(uint8_t dir)
{
	...
	switch (dir) {
		case 2:
			...
			break;
		case 3:
			lcddev.width=Y_PIXEL; //800;
			lcddev.height=X_PIXEL; //480;
			break;
		case 4:
			lcddev.width=X_PIXEL; //480;
			lcddev.height=Y_PIXEL; //800;
...
STATIC void tftlcd_lcd43g_print(const mp_print_t *print, mp_obj_t self_in, mp_print_kind_t kind)
{
	mp_printf(print, "LCD43R(portrait=%d),width:%u,height:%u,X_PIXEL:%u,Y_PIXEL:%u\n",
		lcddev.dir, lcddev.width, lcddev.height, lcddev.x_pixel, lcddev.y_pixel);
}
...
STATIC mp_obj_t tftlcd_lcd43g_make_new(const mp_obj_type_t *type, size_t n_args, size_t n_kw, const mp_obj_t *all_args)
{
	...
	lcddev.x_pixel = X_PIXEL; //480;
	lcddev.y_pixel = Y_PIXEL; //800;
...
{
//	Lcd_ColorBox(x, y, width, height, ~color); //{} tst
...
mp_obj_t tftlcd_lcd43g_drawRect(size_t n_args, const mp_obj_t *pos_args, mp_map_t *kw_args)
{
	...
	if(args[6].u_obj != mp_const_none && args[6].u_obj != MP_OBJ_NULL) {
		...
		for(uint16_t i = 0 ; i <= (args[3].u_int - (args[5].u_int * 2)); i++)
			LCD43G_DrawLine(
							args[0].u_int + args[5].u_int,
							args[1].u_int + args[5].u_int + i,
							args[0].u_int + args[2].u_int - args[5].u_int,
							args[1].u_int + args[5].u_int + i,
							color
			);
		}
	...
	if (args[0].u_obj != MP_OBJ_NULL) {
		...
		if (mp_obj_is_int(args[0].u_obj)) {
			...
			if (text_size == 1)
				...
				mp_raise_ValueError(MP_ERROR_TEXT("lcd size parameter error"));
	...
	if (args[0].u_obj != MP_OBJ_NULL) {
		...
		if (mp_obj_is_int(args[0].u_obj)) {
			...
			if (args[1].u_obj != MP_OBJ_NULL) {
				...
				snprintf(path_buf, 50, "%s", path);
			} else {
				snprintf(path_buf, 50, "%s%s", file_path, ".cache");
			}
			//------------------------------------------------
			UINT bw;
			//FIL		*f_file;
